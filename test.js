// Title : uinttest for smartcontract(LegionX)
//BY ENG: Jamill hallak
//CC for SIMON COMPANY

//note : must run allowUnlimitedContractSize in ganache-cli

// GET smartcontract artifacts ,necessary library 
const legionX= artifacts.require("LegionX");
const utils = require("./helper");
const { MerkleTree } = require('merkletreejs');
const keccak256 = require('keccak256');


contract("LegionX", (accounts) => {
...
let url = "https://my-json-server.typicode.com/abcoathup/samplenft/tokens/";
    ...
    //JavaScript object that will act as an instance of the contract.
      contractInstance = await legionX.new("Mynft","nft",2,2,"i",url,whitelistAddresses,[24,24,4,5],{from: alice});
    });

//check if the contract deployed
    it("Contract deployment", function() {

        return legionX.deployed().then(function (instance) {
          contractInstance = instance;

          assert(contractInstance !== undefined, 'legionX contract should be defined');
        });
    });
...
context("Pre sale mint scenario", async () => {
        ...
        it("should not allow to presalemint,cause : The public sale has started !",async () => {
            ...
            await utils.shouldThrow(contractInstance.Presalemint(1,hexProof,{value:2}))
        })
        ...
        it("should not allow to presalemint,cause : wallet limit exceeded  !",async () => {
            ...
            await utils.shouldThrow(contractInstance.Presalemint(5,hexProof,{value:2}))
        })
        ...
        it("should not allow  to presalemint , cause: Invalid mint amount ,smaller than zero !", async () => {
             ...
             await utils.shouldThrow(contractInstance.Presalemint(-7,hexProof,{value:2}))
        })

        it("should not allow  to presalemint , cause: Invalid mint amount ,greater than MaxMintAmountPerTx !", async () => {
                   
            await contractInstance.setPaused(true,{from: alice })
            await contractInstance.setMerkleRoot((rootHashBytes32),{from: alice} )
            await contractInstance.setMaxMintAmountPerTx(1,{from : alice})

            await utils.shouldThrow(contractInstance.Presalemint(2,hexProof,{value:2}))
        })
        ...
        it("should not allow  to presalemint , cause: Max supply exceeded ! ", async () => {
            ...
            await utils.shouldThrow(contractInstance.Presalemint(34,hexProof,{value:2}))
       })
        ...
        it("should not allow  to mint , cause: Incorrect amount : no value sent with Tx ! ", async () => {
            ...
            await utils.shouldThrow(contractInstance.Presalemint(1,hexProof))
       })
...
context(" mint scenario", async () => {
        ...
        it("should not allow  to mint , cause: public sale not active yet!", async () => {
             ...
             await utils.shouldThrow(contractInstance.mint(1,{value:2,from : alice}))
        })
        ...
        it("should not allow  to mint , cause: Invalid mint amount ,smaller than zero !", async () => {
            ...
            await utils.shouldThrow(contractInstance.mint(-2,{value:2,from : alice}))
        })

        it("should not allow  to mint , cause: Invalid mint amount ,greater than MaxMintAmountPerTx !", async () => {
        ...
        it("should not allow  to mint , cause: Max supply exceeded ! ", async () => {
            ...
            await utils.shouldThrow(contractInstance.mint(34,{value:2,from : alice}))
        })
...
context("walletOfOwner and mint scenario", async () => {
        ...
        it("should get walletOfOwner after 1 mint process ", async () => {
            ...
            assert.equal(result.toString(),[0].toString(),"comparison error")
        })
        ...
        it("should get walletOfOwner after several mint process ", async () => {
                ...
                assert.equal(result.toString(),[0,1,2,3,4,5].toString(),"comparison error")
        })
    })
...
context("tokenURI scenario", async () => {
        ...
        it("should get tokenURI ", async () => {
            ...
            await contractInstance.setPaused(false,{from: alice })
            await  contractInstance.mint(1,{value:2,from:alice})
            result=await contractInstance.tokenURI(0)
...
context("set scenario", async () => {
        ...
        it("should be able to set a cost", async () => {
            ...
            result=await contractInstance.getCost({from:alice})
            //expect(result.receipt.status).to.equal(true);

            assert.equal(result.toNumber(), 2, 'unable to set  ');
        })

        it("should be able to set a Revealed to true", async () => {
         await contractInstance.setRevealed({from : alice })
         result = await contractInstance.getRevealed({from : alice})

         assert.equal(result,true,"set failed")
        })

        it("should be able to setMaxMintAmountPerTx", async () => {
            await contractInstance.setMaxMintAmountPerTx(100,{from : alice})
            result= await contractInstance.getMaxMintAmoutPerTx({from:alice })

            assert.equal(result,100,"unable to set MaxMintAmountPerTx")
        })

        it("should be able to set a HiddenMetadataUri ", async () => {
             await contractInstance.setHiddenMetadataUri("sdf",{from : alice })
             result = await contractInstance.getHiddenMetadataUri()
        ...
        it("should be able to set BaseUrl", async () => {
             ...
             result= await contractInstance.getUri()

             assert.equal(result,"https://my-json-server.typicode.com/abcoathup/samplenft/tokens/","unalbe to set Base Url,must be equal ")
        })

        it("should be able to set Paused to true", async () => {
            await contractInstance.setPaused(true,{from: alice })
            result = await contractInstance.getPaused()

            assert.equal(result,true,"unable to set Pasued")
        })

        it("should be able to set MarkelROOT ", async () => {
            await contractInstance.setMerkleRoot((rootHashBytes32),{from: alice} )
            result= await contractInstance.getMerkleRoot()

            assert.equal(result,rootHashBytes32.toString(),"unable to set markelRoot")
        })
    })
...
//test for release 
context("Release scenario", async () => {

        it("should able to release",async () => {
           await contractInstance.setPaused(false,{from: alice })
           await  contractInstance.mint(1,{value:2,from:alice})
           await  contractInstance.mint(1,{value:2,from:bob})
    
           await contractInstance.release(bob,{from : alice})
        })

        it("should not allow to release,cause : no permission",async () => {
            await contractInstance.setPaused(false,{from: alice })
            await  contractInstance.mint(1,{value:2,from:alice})
            await  contractInstance.mint(1,{value:2,from:bob})

            await utils.shouldThrow(contractInstance.release(alice,{from : bob}))
        })

        it("should not allow to release,cause : account is not due payment",async () => {
            await contractInstance.setPaused(false,{from: alice })

            await utils.shouldThrow(contractInstance.release(bob,{from : alice}))
        })
    })
